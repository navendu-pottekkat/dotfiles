...
# Utkarsh Srivastava's personal changes



run-fast:

	cd cmd; go clean; go mod tidy; \
	...
	go run -ldflags="-X main.version=${GIT_VERSION} -X main.commitsha=${GIT_COMMITSHA} -X main.releasechannel=${RELEASE_CHANNEL}" main.go; \
...
run-fast-stable:
	...
	go run -ldflags="-X main.version=${GIT_VERSION} -X main.commitsha=${GIT_COMMITSHA} -X main.releasechannel=stable" main.go; \
...
run-fast-edge:
	...
	go run -ldflags="-X main.version=${GIT_VERSION} -X main.commitsha=${GIT_COMMITSHA} -X main.releasechannel=edge" main.go; \

	cd ..



# this is for the new plugins startup process
...
run-fast-plugins-dev:
	...
	go run -ldflags="-X main.version=${GIT_VERSION} -X main.commitsha=${GIT_COMMITSHA} -X main.releasechannel=${RELEASE_CHANNEL}" -trimpath main.go; \
